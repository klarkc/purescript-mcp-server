...
validateProtocolVersion clientVersion
  ...
  | clientVersion == "2024-11-05" = Right "2024-11-05"        -- Accept this version
  | otherwise = Left $ "Unsupported protocol version: " <> clientVersion <> ". Server supports: " <> protocolVersion
...
runMcpServer :: (MonadIO m)
             ...
             -> m ()
runMcpServer serverInfo handlers outputHandler inputHandler = do
  loop
  where
    loop = do
      input <- inputHandler
              ...
              Right message -> do
                ...
                case response of
                  Just responseMsg -> do
                    ...
                    outputHandler $ TE.decodeUtf8 $ BSL.toStrict $ encode $ encodeJsonRpcMessage responseMsg
...
runMcpServerStdIn serverInfo handlers =
  runMcpServer serverInfo handlers outputHandler inputHandler
  where
    outputHandler text = do
      TIO.putStrLn text
      hFlush stdout
    inputHandler = TIO.getLine
